...
void OOJDistributionComparisonNew(Bool_t ishhCorr=0, Float_t PtTrigMin1=3, Float_t PtTrigMax1 =15, Float_t PtTrigMin2=0.15, Float_t PtTrigMax2 =2.5, Bool_t SkipAssoc=1,Int_t israp=0, Int_t sysV0=0, Int_t sysTrigger=0,Int_t sys=0,Int_t type=8, Int_t PtIntervalShown=1,   TString year0 = "2016",TString year=/*"AllMC_hXi"/*"2016kehjl_hK0s"/*"2016k_hK0s"*/"Run2DataRed_MECorr_hXi"/*"2016k_hK0s_30runs_150MeV"*/, TString yearMC="AllMC_hXi"/*"2018f1_extra_hK0s"/*"2016kl_hXi"/*2018f1_extra_hK0s_30runs_150MeV"*/,  TString Path1 ="_Jet0.75"/*"_PtTrigMax2.5_Jet0.75"/*"_NewMultClassBis_Jet0.75"*/, TString Path2 =""/*"_PtTrigMax2.5"/*"_NewMultClassBis_Jet0.75"*/, TString Dir ="FinalOutput/", Bool_t isEnlargedDeltaEta=0, Int_t isMC=0, Int_t MultBinning=0, Int_t PtBinning=0){
  ...
  for (Int_t LoopFile=0; LoopFile<=1; LoopFile++){
	...
	for(Int_t PtTrig=0; PtTrig<numPtTrig; PtTrig++){
	  ...
	  for (Int_t IntisMC=LimInfMC; IntisMC<=LimSupMC; IntisMC++){
	    ...
	    PathIn[PtTrig]+= hhCorr[ishhCorr]+ Form("_SysT%i_SysV0%i_Sys%i_PtMin%.1f", sysTrigger, sysV0, sysang, PtTrigChosen)+"_Output.root";
      ...
      //the histogram above is equal to the histogram 'JetNewMethodRebinSub' since rebinning -> subtracting == subtracting --> rebinning (only a difference in the error if histograms involve in subtraction are treated as fully correlated. In this case I treat them as uncorrelated (...not correct...) and therefore there are no differences
	    ...
	    if (LoopFile==0)	{
	      ...
	      hDeltaEtaDeltaPhi_PhiProjBulk[m][v][PtTrig]   ->GetYaxis()->SetRangeUser(0,LimSupY[ishhCorr]);
	      hDeltaEtaDeltaPhi_PhiProjBulkRatio[m][v][PtTrig] ->Divide(hDeltaEtaDeltaPhi_PhiProjBulk[m][v][PtTrig]);
	    ...
	    if (LoopFile==1)	 {
	      canvasPlotOOJDistr[m][IntisMC][1]->cd(v);
	      hDeltaEtaDeltaPhi_PhiProjBulkRatio[m][v][PtTrig]->GetYaxis()->SetRangeUser(0.7, 1.4);
	      hDeltaEtaDeltaPhi_PhiProjBulkRatio[m][v][PtTrig]->Draw("");
	      tlineAtOneAllDeltaPhi->Draw("same"); 
	      canvasPlotOOJDistr[m][IntisMC][1]->cd(v+4);
	    ...
	    if (LoopFile==1){
	      canvasPlotOOJDistrMultComp[IntisMC]->cd(v);
	      ...
	      canvasPlotOOJDistrMultComp[IntisMC]->cd(v+4);
