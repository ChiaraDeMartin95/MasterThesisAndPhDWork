...

{
	...
	*/
	...
	auto binTriggClass = AxisStr("TriggClass", {"HMMBAND","HMMBAND300", "MBAND015", "MBAND300"}); //remove unneccessary bins
...
void AliAnalysisPseudoRapidityDensityTemp::UserExec(Option_t *)
{
	...
	if (mcEvent)
	{
		...
		for (Int_t i = 0; i < nPrim; i++)
		{
			...
			if (bevtc[kINEL] && fabs(eta) < 0.8 && pt > 2)
				bevtc[kINEL200] = true;
			if (bevtc[kINEL] && fabs(eta) < 0.8 && pt > 3) // To Chiara: This is OK because it is the event condition you are saying
				bevtc[kINEL300] = true;
	...
	if (IsGoodVertexCut)
	{
		...
		for (auto it = 0; it < fEvt->GetNumberOfTracks(); it++)
		{
			...
			if (IsMinimumBias && fabs(eta) < 0.8 && pt > 3)  // To Chiara: This is OK because it is the trigger condition you are talking about
				btrigc[kMBAND300] = true;
			if (btrigc[kHMMBAND] && fabs(eta) < 0.8 && pt > 3)
