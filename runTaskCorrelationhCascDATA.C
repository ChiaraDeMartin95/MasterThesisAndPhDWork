...
void runTaskCorrelationhCascDATA(
	     ...
	     const char *taskname = "Xijetpp_LHC18f1_extra_Hybrid_10runs_Rlabel"   // sets name of grid generated macros
	     ){
  // check run type
  ...
  */
  ...
  AliAnalysisTaskCorrelationhCascDATA *task = AddTaskCorrelationhCascDATA(3,30, isLocal, isMC, isEff,isHybridMCTr, 10, EtaTrigger, EtahAssoc,EtaV0Assoc, FilterBitValue, year, "Xi", isHM, "name");
...
AliAnalysisGrid* CreateAlienHandler(const char *taskname, const char *gridmode, const char *proofcluster, const char *proofdataset)
{
  ...
  //  plugin->SetDataPattern("/pass2/AOD208/*/AliAOD.root");  AOD208 are not there anymore
  ...
  plugin->SetAnalysisSource("AliAnalysisCorrelationEventCollection.cxx  AliAnalysisTaskCorrelationhCascDATA.cxx");
  ...
  plugin->SetAdditionalLibs("libOADB.so AliAnalysisCorrelationEventCollection.h AliAnalysisCorrelationEventCollection.cxx AliAnalysisTaskCorrelationhCascDATA.h AliAnalysisTaskCorrelationhCascDATA.cxx");
  ...
  //plugin->SetOutputFiles("AnalysisResultsLocalhCascDATA.root");
