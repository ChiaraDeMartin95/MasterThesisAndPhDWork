...
void CompareDatavsMC_FigureForPaper( Int_t PlotType =0, Int_t ChosenRegion = -1,  Float_t ScalingFactorXiK0s = 1/*0.8458/1.08747*/, Bool_t isChangesIncluded=1, Bool_t isFit=0, Bool_t isdNdEtaTriggered=1, Bool_t MaterialBudgetCorr=1){
  ...
  Float_t Yields[numRegions][numColls][numDataorMC][nummoltMax];
  ...
  */
  ...
  else   smalllegendMCTypes = new TLegend(0.14, 0.68, 0.27, 0.92);
  ...
  else   hdummy->GetYaxis()->SetNdivisions(505, "false");
  hdummy->GetYaxis()->CenterTitle(true);

  if (PlotType==0){
    hdummy->GetYaxis()->SetTitleOffset(2.4);
  }
  ...
  for (Int_t ireg=0; ireg<numRegions; ireg++){
	...
	if (isMC!=0){
	  ...
	  ghistoYieldRatioDATA[ireg][Coll][isMC]->SetFillStyle(1001);
	  ghistoYieldRatioDATA[ireg][Coll][isMC]->SetFillColor(ColorDiffPale[ireg][Coll]);
  ...
  if (kTRUE){
    ...
    SetTickLength(hdummyRatioToOOJ, tickXLL, tickYLL);
    hdummyRatioToOOJ->GetYaxis()->SetNdivisions(205, "false");
    hdummyRatioToOOJ->GetYaxis()->CenterTitle(true);
    if (PlotType==0){
      hdummyRatioToOOJ->GetYaxis()->SetTitleOffset(2.4);
    }
