...
void confronto_histo_thesis(Bool_t isSignalFromIntegral=0, Bool_t isBkgParab=0, Bool_t isMeanFixedPDG=1, Float_t PtTrigMin=3.0, Int_t type=4, Int_t sysTrigger=0, Int_t sysV0=11, Int_t sys=0, TString yearMC="2016kl", TString yeardati="Run2DataRed_hXi", TString year="Run2DataRed_hXi", TString year0="2016", Bool_t isParticleStudy=1, Bool_t isSystV0Analysis=0, Int_t rap=0, TString Path1="", Bool_t SkipAssoc=1){ 
  ...
  TDirectoryFile *dir = (TDirectoryFile*)filedatipubblicati->Get("Table 3");
	...
	for (Int_t i =ForLoopLimInf ; i>= ForLoopLimit; i--){//ok for mult loop
	  ...
	  if (!SkipIsto)	{
		...
		for(Int_t b=1; b <=  histo[j][i]->GetNbinsX(); b++){
		  ...
		  histo_ratio[j][i]->SetBinError(b, sqrt(pow(histo[j][i]->GetBinError(b),2) +pow(MeanRelErr[i]* histo[j][i]->GetBinContent(b),2)) /splineFio[i]->Eval(histo[j][i]->GetBinCenter(b)));
	  ...
	  //if particle selection dependence is analyzed*****************
	  if (dataorMC==1 && isSystV0Analysis && counterParticleType==1 && i!=0) legend->AddEntry(histo[j][i],systematic[i],"pel");
	  if (dataorMC==1 && isSystV0Analysis && counterParticleType==1 && i!=0) legendMass->AddEntry(histo[j][i],systematic[i],"pel");
	  if(i==0 && j==1 && dataorMC==0 && counterParticleType==1) legendMass->AddEntry(retta, tipoNotSign[ParticleType]+" PDG mass");
	  outfile->cd();
	  // histo[j][i]->Write();
	  // histo_ratio[j][i]->Write();
