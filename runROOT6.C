...
void runROOT6(
	const char *taskname = "Mul", const char *option = "LHC16l" // when scanning AOD, add "AOD"
	,
	...
	const char *mode = "grid") //or local
{
	...
	if (foption.Contains("LHC16q"))
	{
		...
		if (foption.Contains("MC"))
		{
			plugin->SetGridDataDir("/alice/sim/2020/LHC20f11c_fast");
			//plugin->SetGridDataDir("/alice/sim/2015/LHC15g6d/1/");
			plugin->SetDataPattern("*/AliESDs.root");
		}
		isaa = true;
	...
	plugin->SetGridWorkingDir(Form("5TeVChiaraTryWithFixwLessBins%s%s", taskname, option));
