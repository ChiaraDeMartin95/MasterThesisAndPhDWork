...
void AliAnalysisTaskMyTask::UserExec(Option_t *)
{
  ...
  */
  ...
  //yes1 
  if(Evcounter==1) FifoShiftok=kTRUE;
    
  //yes1 
  if (FifoShiftok==kTRUE)  fEventColl[zBin][centralityBin]->FifoShift();
  ...
  TClonesArray* AODMCTrackArray =0x0;  
	...
	for(Long_t i = 0; i < AODMCTrackArray->GetEntriesFast(); i++) {
	  ...
	  if(trParticle->Pt()<= fminPtj && trParticle->Pt()>=fmaxPtj)continue;
	   ...
	   fEvt->fReconstructedFirst[NumberFirstParticle].fCharge       = trParticle->Charge();
	   fEvt->fReconstructedFirst[NumberFirstParticle].fPt           = trParticle->Pt();
	   fEvt->fReconstructedFirst[NumberFirstParticle].fEta          = trParticle->Eta();
	   fEvt->fReconstructedFirst[NumberFirstParticle].fPhi          = trParticle->Phi();
	   fEvt->fReconstructedFirst[NumberFirstParticle].fTheta        = trParticle->Theta();
	   fEvt->fReconstructedFirst[NumberFirstParticle].fDCAz         = 0;
	   fEvt->fReconstructedFirst[NumberFirstParticle].fDCAxy        = 0;
	   fEvt->fReconstructedFirst[NumberFirstParticle].fMultiplicity = lPercentiles;
	   fEvt->fReconstructedFirst[NumberFirstParticle].fZvertex      = lBestPrimaryVtxPos[2];
	   fEvt->fReconstructedFirst[NumberFirstParticle].isP           = labelPrimOrSec;
	   NumberFirstParticle++;
  ...
  for(Int_t i(0); i < V0Tracks; i++) {       
    ...
    */
    ...
    for(Int_t j=0; j < NumberFirstParticle; j++){
      if (v0->Pt() >= fEvt->fReconstructedFirst[j].fPt){
	skipV0=kTRUE;
	//	  continue;
      }
    }
    ...
    if(!fReadMCTruth){
     fEvt->fReconstructedSecond[NumberSecondParticle].sDcaPosV0     = v0->DcaPosToPrimVertex();
     fEvt->fReconstructedSecond[NumberSecondParticle].sDcaNegV0     = v0->DcaNegToPrimVertex();
     fEvt->fReconstructedSecond[NumberSecondParticle].sPtArmV0      = v0->PtArmV0();
     fEvt->fReconstructedSecond[NumberSecondParticle].sAlphaV0      = v0->AlphaV0();
     fEvt->fReconstructedSecond[NumberSecondParticle].sInvMassK0s   = v0->MassK0Short();
     fEvt->fReconstructedSecond[NumberSecondParticle].sInvMassLambda   = v0->MassLambda();
     fEvt->fReconstructedSecond[NumberSecondParticle].sInvMassAntiLambda = v0->MassAntiLambda();
     fEvt->fReconstructedSecond[NumberSecondParticle].sCosPointingAngle  = v0->CosPointingAngle(lBestPrimaryVtxPos);
     fEvt->fReconstructedSecond[NumberSecondParticle].sDcaV0ToPV    = v0->DcaV0ToPrimVertex();
     fEvt->fReconstructedSecond[NumberSecondParticle].sRap          = rapidityV0[0];
     fEvt->fReconstructedSecond[NumberSecondParticle].sPt           = v0->Pt();
     fEvt->fReconstructedSecond[NumberSecondParticle].sEta          = v0->Eta();
     fEvt->fReconstructedSecond[NumberSecondParticle].sTheta        = v0->Theta();
     fEvt->fReconstructedSecond[NumberSecondParticle].sPhi          = v0->Phi();
     fEvt->fReconstructedSecond[NumberSecondParticle].isP           = labelPrimOrSecV0;
     fEvt->fReconstructedSecond[NumberSecondParticle].sPDGcode      = V0PDGCode;
  ...
  if(fReadMCTruth){
	...
	for(Int_t j=0; j < NumberFirstParticle; j++){
	  if (particleV0->Pt() >= fEvt->fReconstructedFirst[j].fPt){
	    skipV0_MC=kTRUE;
	  }
	}
	if (skipV0_MC)      continue;
   	 
	fEvt->fReconstructedSecond[NumberSecondParticle].sDcaPosV0     = 0;
	fEvt->fReconstructedSecond[NumberSecondParticle].sDcaNegV0     = 0;
	fEvt->fReconstructedSecond[NumberSecondParticle].sPtArmV0      = 0;
	fEvt->fReconstructedSecond[NumberSecondParticle].sAlphaV0      = 0;
	fEvt->fReconstructedSecond[NumberSecondParticle].sInvMassK0s   = 0;
	fEvt->fReconstructedSecond[NumberSecondParticle].sInvMassLambda   = 0;
	fEvt->fReconstructedSecond[NumberSecondParticle].sInvMassAntiLambda = 0;
	fEvt->fReconstructedSecond[NumberSecondParticle].sCosPointingAngle  = 0;
	fEvt->fReconstructedSecond[NumberSecondParticle].sDcaV0ToPV    = 0;
	fEvt->fReconstructedSecond[NumberSecondParticle].sRap          = particleV0->Y();
	fEvt->fReconstructedSecond[NumberSecondParticle].sPt           = particleV0->Pt();
	fEvt->fReconstructedSecond[NumberSecondParticle].sEta          = particleV0->Eta();
	fEvt->fReconstructedSecond[NumberSecondParticle].sTheta        = particleV0->Theta();
	fEvt->fReconstructedSecond[NumberSecondParticle].sPhi          = particleV0->Phi();
	fEvt->fReconstructedSecond[NumberSecondParticle].isP           = 1;
	fEvt->fReconstructedSecond[NumberSecondParticle].sPDGcode      = particleV0->GetPdgCode();
