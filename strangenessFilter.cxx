...
struct strangenessFilter {
  ...
  // Tables
  //  using CollisionCandidates = soa::Filtered<soa::Join<aod::Collisions, aod::EvSels, aod::CentV0Ms>>::iterator; for Run3 data centrality is not defined
  using CollisionCandidates = soa::Filtered<soa::Join<aod::Collisions, aod::EvSels>>::iterator;
  ...
  void processRun2(CollisionCandidates const& collision, TrackCandidates const& tracks, Cascades const& fullCasc, aod::V0Datas const& V0s, DaughterTracks& dtracks)
  {
    ...
    for (auto& casc : fullCasc) { // loop over cascades
      /*
      //      auto v0 = casc.v0data();      //not compiling
      //      auto v0 = casc.v0_as<aod::V0Datas>(); //crashing
      //      auto v0 = casc.v0_as<aod::V0s>(); //crashing
  ...
  void processRun3MC(MCCollisionCandidates const& collision, TrackCandidates const& tracks, Cascades const& fullCasc, aod::V0Datas const& V0s, DaughterTracks& dtracks)
  {
    if (kint7 && !collision.alias()[kINT7]) {
      return;
    }
