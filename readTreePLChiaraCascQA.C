...
Double_t fparab(Double_t *x, Double_t *par)
{
  ...
  Float_t liminf[numPart]={ 1.31,  1.31,  1.665, 1.665, 1.31, 1.665};
  Float_t limsup[numPart]={ 1.335, 1.335, 1.685,  1.685, 1.335, 1.685};
  Int_t part=par[3];
...
void readTreePLChiaraCascQA( Int_t type=4 /*type = 0 for XiMinus, =1, for XiPlus, =2 for OmegaMinus, =3 for OmegaPlus */,Bool_t SkipAssoc=1 ,Int_t israp=0, Bool_t ishhCorr=0, Float_t PtTrigMin=3, Float_t ptjmax=15, Int_t sysV0=0, bool isMC = 0,Bool_t isEfficiency=1,Int_t sysTrigger=0,			    TString year="2016k_hXi", TString year0="2016", TString Path1 ="", Bool_t AllRun2=1)
{
    ...
    Float_t min_range_signal[numtipo]={ 1.315, 1.315, 1.668, 1.668, 1.315, 1.668};
    Float_t max_range_signal[numtipo]={ 1.328, 1.328, 1.677, 1.677, 1.328, 1.677};
    Float_t liminf[numtipo]={ 1.30, 1.30, 1.65, 1.65, 1.30, 1.65};
    Float_t limsup[numtipo]={ 1.35, 1.35, 1.70, 1.70, 1.35, 1.70};
    TF1 *gauss;
    TF1 *bkgparab;
    TF1 *total;
    TF1 *totalbis;
  ...
  for (Int_t child =1; child<=35 ; child++){
    ...
    */
    ...
    hInvMassTotalQA ->GetYaxis()->SetRangeUser(0, 0.0008);
