...
void runTaskCorrelationhhK0sROOT6(
				  const char* corrtype = "", //hh for hadron-hadron correlation
				  ...
				  const char *taskname = "K0sjetpp_LHC16k_FioComp" //LHC18f1_extra_15runsNOFB_150MeV"*/   // sets name of grid generated macros
...
)
{
  ...
  if (strcmp(corrtype, "hh")==0)  ishhCorr = kTRUE;
  ...
  */
  ...
  AliAnalysisTaskCorrelationhhK0s *task;
#if !defined (__CINT__) || defined (__CLING__)
  gInterpreter->LoadMacro("AliAnalysisTaskCorrelationhhK0s.cxx++g");
  gInterpreter->LoadMacro("AliAnalysisCorrelationEventCollection.cxx++g");
  task = reinterpret_cast<AliAnalysisTaskCorrelationhhK0s*>(gInterpreter->ExecuteMacro(Form("AddTaskCorrelationhhK0s.C(%.2f, %d, %i, %i, %i, %i, %i)", 0.15, 15, 0, islocal, isMC, isEff, isHybridMCTr)));  
#else
  ...
  if(islocal) {
    ...
    alienHandler->SetAdditionalLibs("libAliPythia6 libpythia6_4_21.so AliAnalysisCorrelationEventCollection.h AliAnalysisTaskCorrelationhhK0s.h");
    alienHandler->SetAnalysisSource("AliAnalysisTaskCorrelationhhK0s.cxx AliAnalysisCorrelationEventCollection.cxx");
