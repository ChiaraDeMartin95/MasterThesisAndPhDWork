...
void SysPurity( Int_t VariableOfInt=0, Int_t ishhCorr=0, Float_t PtTrigMin =3, Float_t PtTrigMax=15,  Bool_t isMC=0,   Int_t israp=0,TString year="1617_AOD234_hK0s"/*"1617_hK0s"/*"AllMC_hXi"/*"Run2DataRed_hXi"/,  TString Path1 ="_Jet0.75"/*"_NewMultClassBis_Jet0.75"*/, Int_t type=0,  Bool_t isEfficiency=1,   TString Dir="FinalOutput",TString year0="2016", Bool_t SkipAssoc=1, Int_t MultBinning=0, Int_t PtBinning=1){
  ...
  TCanvas *canvasRatioPurity[nummolt+1]; 
  TCanvas *canvasRelError = new TCanvas("canvasRelError","canvasRelError", 1000, 800);
  for (Int_t m=0; m<=nummolt; m++){
    canvasPurity[m] = new TCanvas( Form("canvasPurity_m%i",m), Form("canvasPurity_m%i",m), 1000, 800);
    canvasRatioPurity[m] = new TCanvas( Form("canvasRatioPurity_m%i",m), Form("canvasRatioPurity_m%i",m), 1000, 800);
  }
  ...
  for (Int_t m=0; m<=nummolt; m++){
    ...
    for (Int_t file = 0; file < numInputFiles; file++){
      if (type==0 && (file == 1 || file ==2 || file ==4)) continue; 
      cout << "file " << file << endl;
      ...
      TString InputPath = CommonInputPath + TypeBkg[file] + Form("_molt%i_sysT0_sysV00_Sys0_PtMin%.1f", m, PtTrigMin) + TypeRange[file];
      InputPath+=".root";
      ...
      if (m==0)      legend->AddEntry(hPurity[m][file], SLegend[file], "pl");
      StyleHisto(hPurity[m][file], 0.8, 1, FileColor[file], 33, "p_{T} (GeV/c)", Variable, Variable, 0, 0,0);
	...
	StyleHisto(hRatioPurity[m][file], 0.8, 1.2, FileColor[file], 33, "p_{T} (GeV/c)", "", "Ratio to default purity", 0, 0,0);
    ...
    StyleHisto(hRelError[m], 0, 0.05, ColorMult[m], 33, "p_{T} (GeV/c)", "", "Relative syst. uncertainty", 0, 0,0);
    canvasRelError->cd();
    hRelError[m]->Draw("same");

    fileout->WriteTObject(hRelError[m]);
    fileout->WriteTObject(canvasPurity[m]);
    fileout->WriteTObject(canvasRatioPurity[m]);
  }
