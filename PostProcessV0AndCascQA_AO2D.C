...
void PostProcessV0AndCascQA_AO2D(TString CollType = "pp", Bool_t isMC=false, Int_t RebinTPC=0,
                      ...
                      TString PathIn  = "../Run3QA/LHC22f_pass2/AnalysisResults_Train47130_qa.root", // input file name
		                  TString PathOut = "../Run3QA/LHC22f_pass2/PostProcessing_Train47130_22f",                     // output file name
  ...
  Float_t FractionOutsideV0[NTopV0variables] = {0};
  TString TypeCutV0[NTopV0variables] = {"SymmCut", "SymmCut", "UpperCut", "LowerCut", "LowerCut"};
  
  cout <<"\n\n\e[35mFraction of entries outside expected range\e[39m" << endl;
  for (int var=0; var<NTopV0variables; var++){
      if (TypeCutV0[var] == "UpperCut") {
          FractionOutsideV0[var] = fHistTopV0[var]->Integral(fHistTopV0[var]->GetXaxis()->FindBin(TopVarV0Cuts[var]),fHistTopV0[var]->GetNbinsX() )/  fHistTopV0[var]->Integral(1,fHistTopV0[var]->GetNbinsX());
      }
      else if (TypeCutV0[var] == "LowerCut") {
        FractionOutsideV0[var] = fHistTopV0[var]->Integral(1, fHistTopV0[var]->GetXaxis()->FindBin(TopVarV0Cuts[var]))/ fHistTopV0[var]->Integral(1,fHistTopV0[var]->GetNbinsX());
      }
       else if (TypeCutV0[var] == "SymmCut") {
        FractionOutsideV0[var] = fHistTopV0[var]->Integral(fHistTopV0[var]->GetXaxis()->FindBin(-TopVarV0Cuts[var]), fHistTopV0[var]->GetXaxis()->FindBin(TopVarV0Cuts[var]))/ fHistTopV0[var]->Integral(1,fHistTopV0[var]->GetNbinsX());
        //cout << fHistTopV0[var]->GetXaxis()->FindBin(-TopVarV0Cuts[var]) << " " << fHistTopV0[var]->GetXaxis()->FindBin(TopVarV0Cuts[var]) << endl;
      }
      if (TypeCutV0[var] == "UpperCut") cout << TopVarV0Input[var]<< " < " << TopVarV0Cuts[var]; 
      else if (TypeCutV0[var] == "LowerCut") cout << TopVarV0Input[var]<< " > " << TopVarV0Cuts[var];
      else if (TypeCutV0[var] == "SymmCut") cout << "|" << TopVarV0Input[var] << "| > " << TopVarV0Cuts[var];
      else cout << TopVarV0Input[var] << " (no cut applied) " << endl;
      cout << " Fraction of entries outside range: " <<  FractionOutsideV0[var]<< endl;
    }
  ...
  Float_t FractionOutsideCasc[NTopCascVar] = {0};
  TString TypeCutCasc[NTopCascVar] = {"NoCut", "NoCut", "LowerCut", "LowerCut", "NoCut", "LowerCut", "LowerCut", "NoCut", "UpperCut", "UpperCut", "SymmCut", "LowerCut", "SymmCut", "SymmCut", "NoCut", "NoCut", "NoCut", "NoCut", "NoCut", "NoCut", "NoCut", "NoCut"};
  ...
  for (int i_Canv=0; i_Canv<kNCanvases; i_Canv++){
    ...
    cout <<"\n\n\e[35mFraction of entries outside expected range\e[39m" << endl;
    for (int var=kCumvariblesPerCanvas[i_Canv]; var<kCumvariblesPerCanvas[i_Canv+1]; var++) {
      if (TypeCutCasc[var] == "UpperCut") {
          FractionOutsideCasc[var] = fHistTopCasc1D[var]->Integral(fHistTopCasc1D[var]->GetXaxis()->FindBin(TopVarCascCuts[var]),fHistTopCasc1D[var]->GetNbinsX() )/  fHistTopCasc1D[var]->Integral(1,fHistTopCasc1D[var]->GetNbinsX());
      }
      else if (TypeCutCasc[var] == "LowerCut") {
        FractionOutsideCasc[var] = fHistTopCasc1D[var]->Integral(1, fHistTopCasc1D[var]->GetXaxis()->FindBin(TopVarCascCuts[var]))/ fHistTopCasc1D[var]->Integral(1,fHistTopCasc1D[var]->GetNbinsX());
      }
       else if (TypeCutCasc[var] == "SymmCut") {
        FractionOutsideCasc[var] = fHistTopCasc1D[var]->Integral(fHistTopCasc1D[var]->GetXaxis()->FindBin(-TopVarCascCuts[var]), fHistTopCasc1D[var]->GetXaxis()->FindBin(TopVarCascCuts[var]))/ fHistTopCasc1D[var]->Integral(1,fHistTopCasc1D[var]->GetNbinsX());
        //cout << fHistTopCasc1D[var]->GetXaxis()->FindBin(-TopVarCascCuts[var]) << " " << fHistTopCasc1D[var]->GetXaxis()->FindBin(TopVarCascCuts[var]) << endl;
      }
      if (TypeCutCasc[var] == "UpperCut") cout << TopVarCasc[var]<< " < " << TopVarCascCuts[var]; 
      else if (TypeCutCasc[var] == "LowerCut") cout << TopVarCasc[var]<< " > " << TopVarCascCuts[var];
      else if (TypeCutCasc[var] == "SymmCut") cout << "|" << TopVarCasc[var] << "| > " << TopVarCascCuts[var];
      cout << "; Fraction of entrties outside range: " <<  FractionOutsideCasc[var]<< endl;
    }
...
*/
  ...
  Float_t NPt[num][2] = {{0., 10.}, {0., 1.}, {1., 1.5}, {1.5, 2.}, {2., 2.5}, {2.5, 4.}, {4., 10.}};
  TString SPt[num] = {"0.0-10.0", "0.0-1.0", "1.-1.5", "1.5-2.0", "2.0-2.5", "2.5-4.0", "4.0-10.0"};
  Float_t PtVector[num] = {0., 1., 1.5, 2., 2.5, 4., 10.};
