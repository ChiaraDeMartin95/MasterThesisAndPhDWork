...
void CfrTopoVarO2vsAliPhyscis(Bool_t isScale=0, TString PathIn ="AnalysisResultsESDMultiStrangeQA.root"/*"APQA.root"/*"outputAliPhysics_LHC20i2b_288909.root"*/, TString PathInO2 = /*apass4 results*/"../O2/AnalysisResults_Run3Data1To10_alsofiltersOK.root"/*"O2output_20i2bv4.root"/*"O2output_18ppbig.root"/*"O2output_LHC20i2b_Ter.root" /*"TriggerOutput_loose.root"/*"O2QA.root"/*"outputsO2Physics_LHC20i2b_288909.root"*/, Int_t AliPhysicsOrO2=1){
  ...
  for (Int_t i=0; i<numTopoVar; i++){
    ...
    if (isScale){
      if (AliPhysicsOrO2!=1) legend->AddEntry(hTopoVarAliPhysics[i], Form("AliPhysics, entries: %i", (int)hTopoVarAliPhysics[i]->GetEntries()), "l");
      else if (AliPhysicsOrO2!=2) legend->AddEntry(hTopoVarO2[i], Form("O2, entries: %i", (int)hTopoVarO2[i]->GetEntries()), "l");
    }
    else {
      if (AliPhysicsOrO2!=1) legend->AddEntry(hTopoVarAliPhysics[i], Form("AliPhysics, entries: %i - integral: %i", (int)hTopoVarAliPhysics[i]->GetEntries(), (int)hTopoVarAliPhysics[i]->Integral()), "l");
      else if (AliPhysicsOrO2!=2)  legend->AddEntry(hTopoVarO2[i], Form("O2, entries: %i - integral: %i", (int)hTopoVarO2[i]->GetEntries(), (int)hTopoVarO2[i]->Integral()), "l");
    }
