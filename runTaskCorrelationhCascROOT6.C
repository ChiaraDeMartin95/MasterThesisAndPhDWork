void runTaskCorrelationhCascROOT6(
...
)
{
  ...
  */
  ...
  AliAnalysisTaskCorrelationhCasc *task;
#if !defined (__CINT__) || defined (__CLING__)
  gInterpreter->LoadMacro("AliAnalysisTaskCorrelationhCasc.cxx++g");
  gInterpreter->LoadMacro("AliAnalysisCorrelationEventCollection.cxx++g");
  task = reinterpret_cast<AliAnalysisTaskCorrelationhCasc*>(gInterpreter->ExecuteMacro(Form("AddTaskCorrelationhCasc.C(%.2f, %d, %i, %i, %i, %i)", 0.15, 15, islocal, isMC, isEff, isHybridMCTr)));  
#else
  ...
  if(islocal) {
    ...
    alienHandler->SetAdditionalLibs("libAliPythia6 libpythia6_4_21.so AliAnalysisCorrelationEventCollection.h AliAnalysisTaskCorrelationhCasc.h");
    alienHandler->SetAnalysisSource("AliAnalysisTaskCorrelationhCasc.cxx AliAnalysisCorrelationEventCollection.cxx");
