...
void Yields_from_invmass(TString year = "LHC21k6_MC_pp",
                         TString SPathIn = "../Run3QA/LHC21k6_MC_pp/AnalysisResults_Train48456_v0s_MC.root" /*"../Run3QA/LHC22s_PbPb/CascLHC22spass4_train47592.root" /*../Run3QA/LHC22s_PbPb/V0sLHC22spass4_train47593.root"*/,
                         Int_t part = 0,
                         ...
                         Float_t sigmacentral = 4)
{
  ...
  TH1F *hEvents = (TH1F *)dir->Get("hEventSelection");
  if (!hEvents)
  {
    cout << "hEventSelection not avilable " << endl;
    return;
  }
  Int_t NEvents = hEvents->GetBinContent(3);
  ...
  for (Int_t pt = 0; pt < numPt; pt++)
  {
    ...
    sprintf(fname1, "1f_%d", pt);
    sprintf(fname2, "2f_%d", pt);
    functionsFirst[pt] = new TF1(fname1, "gaus", min_range_signal[part], max_range_signal[part]);
    ...
    functionsSecond[pt] = new TF1(fname1, "gaus", min_range_signal[part], max_range_signal[part]);
    ...
    functions1[pt] = new TF1(fname1, "gaus", min_range_signal[part], max_range_signal[part]);
    ...
    functions2[pt] = new TF1(fname2, "gaus", min_range_signal[part], max_range_signal[part]);
    ...
    char fnamebkg1[numPt];
    sprintf(fnamebkg1, "f%d", pt);
    bkg1[pt] = new TF1(fnamebkg1, "pol1", min_histo[part], max_histo[part]);
    bkg1[pt]->SetLineColor(418);

    char fnamebkg2[numPt];
    sprintf(fnamebkg2, "f%d", pt);
    bkg2[pt] = new TF1(fnamebkg2, "pol2", min_histo[part], max_histo[part]);
    bkg2[pt]->SetLineColor(kBlue);

    char fnameretta[numPt];
    sprintf(fnameretta, "f%d", pt);
    bkgretta[pt] = new TF1(fnameretta, fretta, liminf[part], limsup[part], 3);
    bkgretta[pt]->SetLineColor(418);
    bkgretta[pt]->FixParameter(2, part);

    char fnamebkgparab[numPt];
    sprintf(fnamebkgparab, "f%d", pt);
    bkgparab[pt] = new TF1(fnamebkgparab, fparab, liminf[part], limsup[part], 4);
    ...
    if (UseTwoGauss)
    {
      cout << "\n\e[35mFit with two gauss \e[39m"
           << " Pt: " << pt << endl;
      char fnametotal[numPt];
      sprintf(fnametotal, "f%d", pt);
      if (isBkgParab)
        total[pt] = new TF1(fnametotal, "gaus(0)+gaus(3)+pol2(6)", liminf[part], limsup[part]);
      else
        total[pt] = new TF1(fnametotal, "gaus(0)+gaus(3)+pol1(6)", liminf[part], limsup[part]);
    ...
    if (!UseTwoGauss)
    {
      ...
      sprintf(fnametotal, "f%d", pt);
      if (isBkgParab)
        total[pt] = new TF1(fnametotal, "gaus(0)+pol2(3)", liminf[part], limsup[part]);
      else
        total[pt] = new TF1(fnametotal, "gaus(0)+pol1(3)", liminf[part], limsup[part]);
    ...
    histoYield->SetBinError(pt + 1, 0); // Yield[pt] / NEvents / histoCountsPerEvent->GetBinWidth(pt + 1));
  }
  ...
  TString Soutputfile = "../Run3QA/" + year + "/Yields_" + Spart[part] + "_" + year + ".root"; //"_FrancescaBin.root";
