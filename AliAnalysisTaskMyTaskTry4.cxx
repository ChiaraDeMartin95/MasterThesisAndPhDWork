...
void AliAnalysisTaskMyTask::UserCreateOutputObjects()
{
  ...
  fHistEventMult=new TH1F("fHistEventMult", "fHistEventMult", 12, 0.5, 12.5);
  ...
  fHistEventMult->GetXaxis()->SetBinLabel(11,"Ntrigger>0 && NV0>0");
  fHistEventMult->GetXaxis()->SetBinLabel(12,"NTrigger>1");

  fHistEventV0=new TH1F("fHistEventV0", "fHistEventV0",11, 0.5, 11.5);
  ...
  fHistEventV0->GetXaxis()->SetBinLabel(11,"pT < pT trigger"); 


  fHistTrack=new TH1F("fHistTrack", "fHistTrack", 9, 0.5, 9.5);
  ...
  fHistTrack->GetXaxis()->SetBinLabel(8,"N.trigger");
  fHistTrack->GetXaxis()->SetBinLabel(9,"N.trigger (NV0>0)");
  ...
  fHistTrigger=new TH1F("fHistTrigger", "Number of trigger particle distribution for selected events (also for events with NV0=0)", 20, -0.5, 19.5); // each entry is an event

  fHistTriggerwV0=new TH1F("fHistTriggerwV0", "Number of trigger particle distribution for selected events (for events with NV0>0)", 20, -0.5, 19.5); // each entry is an event
...
void AliAnalysisTaskMyTask::UserExec(Option_t *)
{
  ...
  */
  ...
  //yes1 if(Evcounter==1) FifoShiftok=kTRUE;
    
  //yes1 if (FifoShiftok==kTRUE)  fEventColl[zBin][centralityBin]->FifoShift();
  //try4   fEventColl[zBin][centralityBin]->FifoShift();
  //cout << "6 " << endl;
  //try4 FifoShiftok=kFALSE;
  // // //se decommento, ho seg viol 
  //try4  fEvt = fEventColl[zBin][centralityBin]->fEvt;
  // // cout << "7 " << endl;
  ...
  for(Int_t i=0; i < iTracks; i++) {
	...
	NumberFirstParticle_finale= NumberFirstParticle;     
      ...
      //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fCharge       = track->Charge();
      //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fPt           = track->Pt();
      //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fEta          = track->Eta();
      //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fPhi          = track->Phi();
      //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fTheta        = track->Theta();
      //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fDCAz         = dz[1];
      //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fDCAxy        = dz[0];
      //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fMultiplicity = lPercentiles;
      //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fZvertex      = lBestPrimaryVtxPos[2];
      //try4 fEvt->fReconstructedFirst[NumberFirstParticle].isP           = labelPrimOrSec;
	...
	for(Long_t i = 0; i < AODMCTrackArray->GetEntriesFast(); i++) {
	  ...
	  if(track->Pt()<= fminPtj && track->Pt()>=fmaxPtj)continue;
	  
	  //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fCharge       = trParticle->Charge();
	  //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fPt           = trParticle->Pt();
	  //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fEta          = trParticle->Eta();
	  //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fPhi          = trParticle->Phi();
	  //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fTheta        = trParticle->Theta();
	  //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fDCAz         = 0;
	  //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fDCAxy        = 0;
	  //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fMultiplicity = lPercentiles;
	  //try4 fEvt->fReconstructedFirst[NumberFirstParticle].fZvertex      = lBestPrimaryVtxPos[2];
	  //try4 fEvt->fReconstructedFirst[NumberFirstParticle].isP           = labelPrimOrSec;
	   NumberFirstParticle++;
  ...
  fHistEventMult->Fill(10); 
  if(NumberFirstParticle>1)fHistEventMult->Fill(12); 

  // cout << " caratt part leading " << endl;
  // cout<<       fEvt->fReconstructedFirst[NumberFirstParticle_finale].fCharge << endl;
  // cout<<       fEvt->fReconstructedFirst[NumberFirstParticle_finale].fPt          << endl;
  // cout<<       fEvt->fReconstructedFirst[NumberFirstParticle_finale].fEta         << endl;
  // cout<<       fEvt->fReconstructedFirst[NumberFirstParticle_finale].fPhi         << endl;
  // cout<<       fEvt->fReconstructedFirst[NumberFirstParticle_finale].fTheta       << endl;
  ...
  for(Int_t i(0); i < V0Tracks; i++) {       
    ...
    */
    ...
    //try4    for(Int_t j=0; j < NumberFirstParticle; j++){
      //try4      if (v0->Pt() >= fEvt->fReconstructedFirst[j].fPt){
      //try4	skipV0=kTRUE;
	//	  continue;
    //try4  }
    //try4 }
    ...
    if(!fReadMCTruth){
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sDcaPosV0     = v0->DcaPosToPrimVertex();
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sDcaNegV0     = v0->DcaNegToPrimVertex();
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sPtArmV0      = v0->PtArmV0();
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sAlphaV0      = v0->AlphaV0();
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sInvMassK0s   = v0->MassK0Short();
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sInvMassLambda   = v0->MassLambda();
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sInvMassAntiLambda = v0->MassAntiLambda();
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sCosPointingAngle  = v0->CosPointingAngle(lBestPrimaryVtxPos);
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sDcaV0ToPV    = v0->DcaV0ToPrimVertex();
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sRap          = rapidityV0[0];
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sPt           = v0->Pt();
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sEta          = v0->Eta();
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sTheta        = v0->Theta();
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sPhi          = v0->Phi();
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].isP           = labelPrimOrSecV0;
//try4     fEvt->fReconstructedSecond[NumberSecondParticle].sPDGcode      = V0PDGCode;
  ...
  if(fReadMCTruth){
	...
	//try4for(Int_t j=0; j < NumberFirstParticle; j++){
	//try4  if (particleV0->Pt() >= fEvt->fReconstructedFirst[j].fPt){
	//try4    skipV0_MC=kTRUE;
	//try4  }
	//try4}
	if (skipV0_MC)      continue;
   	 
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sDcaPosV0     = 0;
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sDcaNegV0     = 0;
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sPtArmV0      = 0;
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sAlphaV0      = 0;
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sInvMassK0s   = 0;
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sInvMassLambda   = 0;
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sInvMassAntiLambda = 0;
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sCosPointingAngle  = 0;
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sDcaV0ToPV    = 0;
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sRap          = particleV0->Y();
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sPt           = particleV0->Pt();
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sEta          = particleV0->Eta();
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sTheta        = particleV0->Theta();
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sPhi          = particleV0->Phi();
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].isP           = 1;
//try4 	  fEvt->fReconstructedSecond[NumberSecondParticle].sPDGcode      = particleV0->GetPdgCode();
//try4 
	  NumberSecondParticle++;
	}
      }
  ...
  FifoShiftok=kTRUE;
  fHistEventMult->Fill(11);  
//try4  fEvt->fNumberCandidateFirst = NumberFirstParticle;
//try4  fEvt->fNumberCandidateSecond = NumberSecondParticle;
  ...
  fHistTrack->AddBinContent(9, NumberFirstParticle);
...
//try4  for(Int_t l=0; l< NumberFirstParticle; l++){
//try4    fHistPt->Fill(fEvt->fReconstructedFirst[l].fPt);
//try4    fHistPtvsMult->Fill(fEvt->fReconstructedFirst[l].fPt, lPercentiles);
//try4    fHist_eta_phi->Fill(fEvt->fReconstructedFirst[l].fPhi, fEvt->fReconstructedFirst[l].fEta);
//try4  }
  ...
  */
  //--------------------------------------------------------------
  //try4  DoPairsh1h2((Int_t)lPercentiles, fieldsign, lBestPrimaryVtxPos[2]);  
